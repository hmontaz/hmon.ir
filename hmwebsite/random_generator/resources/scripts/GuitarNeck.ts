...
class GuitarNeck {
	...
	initialize() {
		...
		this.notes = new Array<GuitarNote>();
		this.ui = {
	...
	getFretCX(index: number) {
		const o = this.options;
		if (index == -1) return -o.horizontalPadding / 2;
		var result = 0;
	...
	addNote(stringNumber: number, fretNumber: number, text: string) {
